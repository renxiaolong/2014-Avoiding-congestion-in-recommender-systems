// DWC_2parameter.cpp : 定义控制台应用程序的入口点。
// DWC 算法又叫SeniConductivityHeatS算法，是HeatS在第一步传播的时候有一个参数，第二步没有参数
// 而本程序的目的是研究有两个参数时候，推荐的ranking score 如何变化
...
const int EXISTED_ITEM_NUM = 5640; 
const string SCORE_FILE = "R\\net_SCHeatS_rlist200_part_0";
...
// exp_a 和 exp_b 分别是两步传播时候的参数
void DWC2(int target_uid, vector<node>items, vector<node>users, vector<itemValue> &score, double exp_a, double exp_b)
{
		...
		//		}
		//}
		for (int i_sub=0; i_sub<MAX_IID; i_sub++)
		{
				bool find_source2 = 0; // 标记这个节点是否和有资源的user相连
				for (int u_sub=0; u_sub<int(items[i_sub].ids.size()); u_sub++)
				{
						if (users[items[i_sub].ids[u_sub]-1].value>0)
						{
								find_source2 = 1;
								break;
						}
				...
				if (find_source2)
				{
						...
						for (int u_sub=0; u_sub<int(items[i_sub].ids.size()); u_sub++)
						{
								...
								denominator += pow(double(items[i_sub].ids.size())*double(users[items[i_sub].ids[u_sub]-1].ids.size()), exp_b);
								if (users[items[i_sub].ids[u_sub]-1].value>0)
								{
										// 分子
										numerator += pow(double(items[i_sub].ids.size())*double(users[items[i_sub].ids[u_sub]-1].ids.size()), exp_b)
												* users[items[i_sub].ids[u_sub]-1].value;
								}
						}
						items[i_sub].value = numerator / denominator; // 不用+=
				}
		}
...
int _tmain(int argc, _TCHAR* argv[])
{
		...
		for (double exp_a=0.8; exp_a<=0.8001; exp_a+=0.2)
		{
				for (double exp_b=2; exp_b>=-0.2001; exp_b -= 0.2)
				{
						//modify the output-file name
						string add_temp="";
						stringstream add_name_ss;
						add_name_ss<<exp_a<<exp_b;
						add_name_ss>>add_temp;
						string score_file = SCORE_FILE + "_exp_" + add_temp;
						...
						for (int target_uid=1; target_uid<=MAX_UID; target_uid++)
						{
								...
								DWC2(target_uid, items, users, score, exp_a, exp_b);
						...
						//cout<<"Average RS is : "<< sum/double(allRankScore.size()) << "  \t";

						cout<<exp_a<<", "<<exp_b<<", "<<sum/double(allRankScore.size()) << "  \n";

						ofstream wtRS("S\\rs.csv",ios::app);
						if (!wtRS) cout<<"error creat rs.csv 890!"<<endl;
						wtRS<<exp_a<<", "<<exp_b<<", "<<sum/double(allRankScore.size()) << "  \n";
						wtRS.close();
				}
		}
